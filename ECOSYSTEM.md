...
| Estate → Distributed Hive | The estate's pin of the Hive root: a `hives[]` entry in `estate.json` naming the public copy's commit and the hash of its `PUBLISHED.md` | The estate owner's choice of commit. Hashes prove integrity; authenticity needs the estate owner's signed registry | RAPP proposal 0020 (§§1, 3, 6), a draft (`kody-w/RAPP#133`); RAPP Constitution Article XLVI | candidate; `estate.json` is a placeholder today, and a draft entry pins the root |
...
| 2. Zero drift, clean pull | engineering | done: RAPP's and RAR's non-conformant eggs and RAR's bounded scan are fixed (`kody-w/RAPP#121`, `kody-w/RAR#1116`, both merged); give rapp-model-hive's frames a trusted anchor: its duplicate frames are fixed (`kody-w/rapp-model-hive#2`, merged), and the anchor needs the owner's registry entries; re-sweep until every repo of the RAPP/1 stack is certified (the dogfood); keep the default pull clean: default branches and the LTS install carry only in-force parts; one front door keeps the network unified and tracked; pin the Windows LTS install (G23; only the owner merges its fix) | G23 |
...
**Keep the default pull clean** (phase 2): on 2026-09-26, `git grep -l -i experimental origin/HEAD` counted the files on each default branch that mention “experimental”: rapp-hive-public 342, RAR 50, RAPP 47, rapp-workspace 26, rapp-hive-hub 19, rapp-model-hive 11, rapp-installer 9, hive-hub 6, lisppy 1, rapp-1 0, rapp-work 0, rapp-drift-lint 0, hive-hub-join 0, hive-hub-mcp 0, rapp-hive-hub-join 0. Mentions, not problems; some are legitimate. Recount at lock time. One front door: **proposed:** the installer's “Start here”, where every header links, leads with the RAPP/1 LTS install (`kody-w/rapp-installer#48`, open; only the owner merges it) · **experimental:** 13 of the stack's 15 default branches show their RAPP/1 status badge (12 certified, 1 not yet) and a “Start here” link; `rapp-installer` waits on #48, `rapp-workspace` on G24 · **experimental:** the drift sweep sets each status; the portfolio in `kody-w/rapp-hive-public`, the network's notice board, shows 14 of the stack's 15 certified, and every repo's channel and lifecycle.
