...
| Estate → Distributed Hive | The estate's pin of the Hive root: a `hives[]` entry in `estate.json` naming the public copy's commit and the hash of its `PUBLISHED.md` | The estate owner's choice of commit. Hashes prove integrity; authenticity needs the estate owner's signed registry | RAPP proposal 0020 (§§1, 3, 5), a draft; RAPP Constitution Article XLVI | candidate; `estate.json` is a placeholder today, and a draft pins the root |
