...
steps:
  ...
  - "2: done: RAPP's and RAR's non-conformant eggs and RAR's bounded scan are fixed (`kody-w/RAPP#121`, `kody-w/RAR#1116`, both merged)"
  - "2: give rapp-model-hive's frames a trusted anchor: its duplicate frames are fixed (`kody-w/rapp-model-hive#2`, merged), and the anchor needs the owner's registry entries"
  ...
  - "2: one front door keeps the network unified and tracked; pin the Windows LTS install (G23; only the owner merges its fix)"
  ...
  - "4: switch on the Distributed Hive: accept RAPP proposal 0020 (`kody-w/RAPP#133`), then merge the drafts of beacon 1.1, `estate.json` and the operator's acceptance in RAPP's seed"
