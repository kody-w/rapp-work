...
- Today the seed's one operator is not accepted, and the beacon and `estate.json` are placeholder status documents, so a walk from the seed stops at the operator's placeholder beacon. The estate kit drafted a real beacon 1.1 and an `estate.json` with a `hives[]` entry for the Hive root, on `kody-w/rapp-estate` branch `experimental/rapp1-distributed-hive`; that entry carries an extra `lts_pins` member, which the revised convention's five-member entry does not allow yet. RAPP's branch `experimental/rapp1-network-seed-acceptance` pins that beacon in the seed, then accepts the operator once the owner allows it. Nothing there is signed or merged.
