...
- In the draft convention, each repository, a station, keeps its own member space: its card `.rapp/member.md` and what it shares under `.rapp/shared/`, changed by its own commits. As of 2026-09-26 10:00 UTC, eleven stations of the RAPP/1 stack publish their card on their default branch, all merged that day: RAR, rapp-1, rapp-model-hive, rapp-work, hive-hub, rapp-hive-hub, hive-hub-mcp, hive-hub-join, rapp-hive-hub-join, rapp-drift-lint and lisppy (for example `kody-w/rapp-1#44`). RAPP's is proposed in open pull request `kody-w/RAPP#132`, and the installer's rides in `kody-w/rapp-installer#48`, which only the owner merges.
