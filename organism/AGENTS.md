...
1. Read `ORGANISM.md` first. It holds the whole organism on one page: the graph, every layer, part, crossing, gap and journey, the path to a locked RAPP/1, and what holds everywhere.
2. Change the organism only through its part files: `layers/`, `parts/`, `crossings/`, `gaps/`, `journeys/`, `invariants.md`, `dogfood.md`, `health.md`, `glossary.md` and `lock.md`. Keep one fact per file. To reorganize, move, add, rename or remove files.
...
5. If the builder refuses, fix the file it names. Do not work around the check. A new gap needs its `phase`, `who` and `blocks`, and a `lock.md` step of that phase that names it.
