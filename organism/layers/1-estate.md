...
- No estate declares the Brainstem's kernel yet. RAPP's unsigned `KERNEL_PIN.json` pins its LTS channel at `kody-w/rapp-installer@brainstem-v0.6.9`; the newest `brainstem-v*` tag, `brainstem-v0.6.16`, is what the installer's `main` ships, and nothing pins it yet (G15).
